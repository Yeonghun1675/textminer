...
def extract_paragraph_OLED(file_name, url_text=None, show_property=False, extract_all_property=False, return_file = False, parser=None):
    ...
    with open('./database/property_OLED.json', 'r', encoding='utf-8') as f:
        #pass
        unit_dict_OLED = json.load(f)
        
    with open("./database/abb_list_OLED.json", 'r', encoding='utf-8') as f:
    ...
    with open("./database/chemical_OLED.json", 'r', encoding='utf-8') as f:
    ...
    with open("./database/exclude_OLED.json", 'r', encoding='utf-8') as f:
        exclude = json.load(f)
    ...
    special_unit_dictionary = {'tuple':fr"(?P<NUM>\(\s?({float_re})\s?,\s*({float_re})\s?\))",
                               'luminescence': fr"(?<=^|\s)(?P<NUM>PL|plutoluminescence|EL|electroluminescence)(?=\s|$)",
                               'pH' : r"pH\s?(?P<NUM>[+-]?\d+[.]?\d*)",
                               'solvent': fr"(?<=^|\s)(?P<NUM>{solvent_re})(?=\s|$)"}
...
def extract_table_OLED(filename, url_text=None, show_data=False, write_rawfile=False, output_filename=None,parser=None):
    ...
    return extract_table(filename, url_text, show_data, write_rawfile, output_filename, parser=parser)
...
def to_pd(file_name, url_text = None, show = False, extract_all_property = False, write_table_raw = False, parser=None):
    ...
    text_data = extract_paragraph_OLED(file_name, url_text = url_text, show_property=show, extract_all_property=extract_all_property,
                                      parser=parser)
    table_data = extract_table_OLED(file_name, url_text = url_text, show_data=show, write_rawfile = write_table_raw,
                                   parser=parser)
