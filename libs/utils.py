...
def chemical_database(set_of_cm, cm_dict1 = None, cm_dict2 = None):
    """cm1, cm2 = chemical_database({H2, H2O}, sm1, sm2)
    set_of_cm : set of chemical
    cm_dict1 : Dictionary (key : chemical, value : hashcode)
    cm_dict2 : Dictionary (key : hashcode, value : chemical)
    
    update and return cm_dict1 and cm_dict2 that append abbreviations
    """
    if not isinstance(cm_dict1, dict):
        cm_dict1 = {}
    if not isinstance(cm_dict2, dict):
        cm_dict2 = {}

    
    cm_dict1.update({v:"-c{}-".format(str(i).zfill(6)) for i, v in enumerate(set_of_cm) })
    cm_dict2.update({"-c{}-".format(str(i).zfill(6)):v for i, v in enumerate(set_of_cm) })
    
    return cm_dict1, cm_dict2
