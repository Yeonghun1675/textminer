...
from libs.utils import word_original, sentence_original, small_molecule_database, Update_list_in_dictionary, cut_paragraph, Get, Check
from libs.abbreviation import Abbreviation, make_abbreviation
from libs.unit_database import Unit_database, dimension_analysis
from libs.preprocessing import DocumentTM
...
class Chemical(Word):
    ...
    def value_match(self, new_value, distance=None):
...
def word_nearest(word_list, target, condition = None, consider_phase = True, return_distance = False):
...
def matching_algorithm(sent, database, chemical_type_dict, before_represent_chem = False):
    ...
    for value in character_value:
        ...
        matched_chemical, distance = word_nearest(value.group, chemical_list, lambda t: isinstance(t, Chemical) and t.cem_type == 'chemical',
        ...
        for grouped_value in value.group:
            ...
            matched_chemical.value_match(value, distance)
