...
from libs.utils import Define, Check
...
class Unit_database():
    def __init__(self, chemical = None, smallmolecule = None, ion = None):
        ...
        self.UNIT = ["days", "weeks", "hours", "minutes", "seconds", 'sec', "\u03A9", "day", "week", "hour", "minute", "months", "month", "year", 
                "cycles", "cycle", "years", 'bar', "Torr", 'vol', "mol", "\u2103", "\u2109", 'min', "Pa", 'cd', 'rad',"eV", "cal", 'PPM', 'ppm', "cc", 
                     "CC", 'drop', 'drops', "m", 's', 'h', "K", "\u00B0C", "\u00B0F", "\u00B0", "g", "Hz", 'N', "V", "L", "l", "W", "%", "wt[.]%", "wt%",
                     "A", "F", "\u00C5" , "J", 'C', "M", 'S', 'atm', 'at%']

        self.UNIT_PRX = ["m", "k", "G", "M", "c", "d", "n", "\u03BC", '\u00B5', "T", "P", "p"]

        #self.UNIT_mid = ["\(STP\)", "STP", "cat", '/', 'per']
        self.UNIT_sub = self.ELEMENTS + ["cat", "cats", "-cat", "[-]cat.", "[-]cat.", 'catalyst', "Cat", "CAT", r"\scat\s", "obs", "\(STP\)", "STP"]
        self.UNIT_regex = ""
        self.unit_to_hash = {}
    ...
    def unit_hash(self, unit_text, unit_cem, unit_analysis):
        if unit_text in self.unit_classification:
            ...
            self.hash_to_unit["-u{}000-".format(hash_unit)] = {'unit':unit_text, 'unit_analysis':unit_analysis, 'unit_cem':frozenset()}
            self.count += 1
    ...
    def find_unit(self, string, UNIT_sub = None):
        ...
        UNIT_re = re.compile(fr"(?<=^|\s|\(|\[|\])(?P<num>{num_regex_form})\s?(?P<UNIT>({UNIT_total_regex_form})+)(?=\s|$|[)(.,]|\[|\]|$)")
...
if __name__ == '__main__':
    ...
    print (UD.is_unit("mThy1-hAPPLond/Swe+"))
    print (UD.is_unit("GO"))
    #print (UD.unit_analysis['cm2.0g-1.0'])
    print (dimension_analysis("V1.0"))
