...
from libs.utils import small_molecule_database, Get, Check, word_original
from libs.abbreviation import Abbreviation, make_abbreviation
from libs.unit_database import Unit_database
...
class DocumentTM():
    ...
    def set_dictionary(self, **database):
        ...
        if 'sm1' in database and 'sm2' in database:
            ...
            with open(abs_path/"smallmolecule.json", 'r', encoding='utf-8') as f:
        ...
        if 'ion1' in database and 'ion2' in database:
            ...
            with open(abs_path/"ion_dict.json", 'r', encoding='utf-8') as f:
        ...
        if 'ABB1' in database and 'ABB2' in database:
            ...
            with open(abs_path/"abb_list.json", 'r', encoding='utf-8') as f:
        ...
        if 'exclude' in database:
            self._exclude = database.get('exclude')
        else:
            with open(abs_path/"exclude.json", 'r', encoding='utf-8') as f:
                self._exclude  = json.load(f)
    ...
    def doc(self, parser = None):
        suffix = self._html_name.suffix
        assert parser in [None, 'cde_parser', 'elsevier_xml_parser']
        if suffix == '.pdf':
        ...
        elif parser == 'elsevier_xml_parser':
            self.Para = []
            with open(str(self._html_name), 'r') as f:
                soup = BeautifulSoup(f, 'lxml')
            
            for element in soup.find_all("ce:para"):
                # extract hyperlinke
                for hyperlink in element.find_all(["ce:cross-refs", "ce:cross-ref"]):
                    hyperlink.extract()
    ...
    def get_name(self, name):
        ...
        if tag == 'chemical':
            ...
            return self._unit_database[name]
    ...
    def tokenize_paragraph(self, sentence, cut_off=True, lemma = True, Strange = False):
        ...
        for i, word in enumerate(new_split):
            ...
            if tag == 'chemical' or tag == 'strange':
                ...
                if " " in name:
                    chem_analyize = self.find_chemical_form(name, tag).get('main_material', None)
                else:
    ...
    def find_chemical_form(self, chem_name, tag = 'strange'):
        ...
        end_group = r"|".join(['type', 'based', 'layered', 'coated', 'decorated', 'activated', 'deposited', 'combined', 'supported',
                                'doped','ion', 'cation', 'anion', "N[A-Z]s", 'metal', 'metallic', 'bimetallic'])
        
        front_group = r"|".join(['pristine', 'bare', 'bared', 'pure', 'modified', 'stablized', '1D', '2D', '3D', '0D', '1-D', '2-D', '3-D',
                                 '0-D', 'metal', 'metallic', 'bimetallic', 'g', 'SP'])
        ...
        output = {'main_material':None, 'sub_material':None, 'chem_type':[], 'ABB':{}}
        ...
        for chem in chem_iter:
            ...
            if chem in ['in', 'on', 'to', 'with']:
                ...
                chem_split = re.sub(r"(?=.+)((?P<split>@)|\d/\d|(?P<split>/)|(?P<split>-))", 
                                    lambda t: " " if t.group("split") else t.group(0), chem).split()
                if len(chem_split)-1:
                    for chem_s in chem_split:
                        if self.is_vaild_chemical(chem_s, 'chemical') or chem_s in self.ELEMENTS_AND_NAMES:
                            chem_list.append('')
                            chem_list[-1] += f' {chem_s}'
                        else:
                            output['chem_type'].append(chem_s)
        ...
        chem_list = [word.strip() for word in chem_list if word]

        if chem_list:
            try:
                index = chem_list.index("|")
                while chem_list[index+1] == '|':
                    index += 1
                output['main_material'] = chem_list[index+1]
            except (ValueError, IndexError):
                output['main_material'] = chem_list[0]
            
            for material in chem_list:
                if material in self._ABB2:
                    output['ABB'][material] = self._ABB2[material]
                
            output['sub_material'] = [chem for chem in chem_list if chem not in (output['main_material'], "|")]
...
if __name__ == '__main__':
    ...
    for i, file in enumerate(files):
        ...
        for word, cl2 in zip(['chemical', 'strange','ion', 'smallmolecule'], chem_list):
            for cl in set(cl2):
                print (cl)
                print (Q.find_chemical_form(cl, word))
